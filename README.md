...
| Package | Description |
|---|---|
| [`@runtime-config/core`](./packages/core) | Types, `defineRuntimeConfig`, env-var overrides |
| [`@runtime-config/vite`](./packages/vite) | Vite plugin — HTML injection, virtual modules, HMR |
| [`@runtime-config/react`](./packages/react) | `useRuntimeConfig` hook, `RuntimeConfigProvider`, SSR utilities |
...
```bash
pnpm add @runtime-config/core @runtime-config/react
pnpm add -D @runtime-config/vite jiti
```
...
import runtimeConfig from '@runtime-config/vite'

export default defineConfig({
  plugins: [
    react(),
    runtimeConfig({ generateTypes: true }),
  ],
})
```
